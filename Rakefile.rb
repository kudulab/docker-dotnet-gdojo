...
def mono_ide_last_tag
  `git ls-remote --tags git@gitlab.ai-traders.com:stcdev/docker-mono-ide.git | sort -t '/' -k 3 -V | awk '{print $2}' | cut -d'/' -f3 | tail -1`.chomp()
end
...
task :save_version do
  ...
  new_contents = text.gsub(/BASE_IMAGE_VERSION/, mono_ide_last_tag())
...
task :save_mono_ide_version do
  mono_ide_last_tag_value = mono_ide_last_tag()
  DockerImageRake::Logging.logger.info("Will docker build from mono-ide:#{mono_ide_last_tag_value}", true)
  dockerfile = "#{image_dir}/Dockerfile"
  text = File.read(dockerfile)
  new_contents = text.gsub(/{{BASE_IMAGE_VERSION}}/, mono_ide_last_tag())
  File.open(dockerfile, "w") {|file| file.puts new_contents }
end
task build: [:save_version, :save_mono_ide_version]
...
namespace 'publish' do
  ...
  task :image do
    ...
    pretty_tag = "#{info.last_git_tag()}_#{mono_ide_last_tag()}"
